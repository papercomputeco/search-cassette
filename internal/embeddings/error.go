...
//
// It unwraps to ErrEmbedding so existing errors.Is(err,
// ErrEmbedding) checks keep working.
...
func (e *APIError) Error() string {
	if e.Status == 0 {
		return fmt.Sprintf("%v: embedding api request failed: %s", ErrEmbedding, e.Message)
	}
	return fmt.Sprintf("%v: embedding api returned status %d: %s", ErrEmbedding, e.Status, e.Message)
}

// Unwrap reports ErrEmbedding so callers can keep matching on the
// package's sentinel.
func (e *APIError) Unwrap() error { return ErrEmbedding }
